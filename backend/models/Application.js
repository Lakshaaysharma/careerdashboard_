...
applicationSchema.virtual('totalExperienceYears').get(function() {
...
applicationSchema.virtual('formattedExpectedSalary').get(function() {
  if (!this.expectedSalary.amount) return null;
  ...
  return `${formatSalary(this.expectedSalary.amount)}/${this.expectedSalary.period}`;
});
...
applicationSchema.statics.findByEmployer = function(employerId, options = {}) {
  ...
  if (options.jobId) {
    query.job = options.jobId;
  }
  
  return this.find(query)
    .populate('job', 'title company location')
    .populate('applicant', 'name email profile')
    .select('applicantName applicantEmail applicantPhone status coverLetter resume expectedSalary experience skills appliedAt interview employerNotes score tags source')
    .sort({ appliedAt: -1 });
};
...
applicationSchema.statics.updateStatus = function(applicationId, status, employerNotes = null) {
  const update = { status };
