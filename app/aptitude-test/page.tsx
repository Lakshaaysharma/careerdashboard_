...
export default function AptitudeTestPage() {
  ...
  // Lock the quiz so questions don't change mid-run
  ...
  const fallbackQuestions = [
    {
      ...
      options: [
        ...
        "Visualize the problem and sketch out solutions",
    ...
    {
      question: "In a team project, which role energizes you the most?",
      options: [
        "The technical architect who designs the system",
        "The creative director who shapes the vision",
        "The team leader who coordinates everyone",
        "The analyst who ensures data-driven decisions",
      ],
      category: "Leadership",
    },
    {
      ...
      options: [
        ...
        "Dynamic, creative spaces with inspiring visuals",
        "Collaborative spaces with constant interaction",
        "Organized spaces with access to resources and data",
      ],
      category: "Work Style",
    },
    {
      ...
      options: [
        ...
        "Study theory first, then apply practically",
      ],
    ...
    },
    {
      ...
      options: [
        ...
        "Discovering insights that inform strategic decisions",
      ],
      category: "Motivation",
    },
    {
      ...
      options: [
        "Create a systematic plan and execute methodically",
        "Focus on the most impactful elements first",
        "Rally the team and delegate effectively",
        "Analyze what's truly essential and prioritize",
      ],
    ...
    },
    {
      ...
      options: [
        "The logical elegance of well-written code",
        "The potential to create engaging user experiences",
        "The ability to connect and empower people",
        "The insights that can be extracted from data",
      ],
      category: "Technology Interest",
    },
    {
      question: "In your ideal job, you would spend most time:",
      options: [
        "Coding, debugging, and optimizing systems",
  ...
  // Local simple banks for known subjects (ensures categories and correct answers)
  ...
  // Load Gemini-generated or local simple questions when subject is provided
  useEffect(() => {
    const loadQuestions = async () => {
      ...
      try {
        ...
        if (data?.success && Array.isArray(data.questions) && data.questions.length > 0) {
          ...
          const normalized = data.questions.map((q: any) => {
            ...
            return {
              question: q.question,
              options: optionTexts.length ? optionTexts : ["A", "B", "C", "D"],
            }
          })
          setDynamicQuestions(normalized)
          // Attempt to derive correctness if present
          const correct = data.questions.map((q: any) => {
            if (q.type === 'true-false') {
              return (String(q.correctAnswer).toLowerCase() === 'true') ? 0 : 1
            }
      ...
      // Fallback to local set if API failed or returned empty
  ...
  const handleNext = () => {
    if (selectedAnswer) {
      ...
      if (currentQuestion < questions.length - 1) {
        ...
        // Generate results and redirect immediately
  ...
  const generateAndStoreResults = (finalAnswers: string[]) => {
    // Compute per-question correctness where available
    ...
    const perQuestionCorrect = finalAnswers.map((ans, idx) => {
      ...
      // Preference-based scoring when correctness is unknown
      ...
      // slight variance to avoid flat 50
      return (idx % 2 === 0) ? 0.6 : 0.4
    })

    // Aggregate by category (fallback to generic categories)
    ...
    const strengths = strengthsSource.slice(0, 3).map(s => ({
      area: s.area,
      score: s.score,
      description: "You performed well in this area compared to others."
    }))
    const weaknesses = weaknessesSource.slice(0, 3).map(w => ({
      area: w.area,
      score: w.score,
      description: "This area needs attention based on your answers.",
      improvement: "Review fundamentals and practice with targeted exercises."
    }))
      ...
      <div className="relative z-10 container mx-auto px-4 py-8 max-w-4xl">
          ...
          <CardContent>
            ...
            <RadioGroup value={selectedAnswer} onValueChange={setSelectedAnswer} className="space-y-4">
              {(questions[currentQuestion]?.options || []).map((option, index) => (
                <div
                  key={index}
                  className="flex items-start space-x-4 p-4 rounded-xl hover:bg-white/5 cursor-pointer transition-all duration-300 border border-transparent hover:border-white/10"
                >
                  <RadioGroupItem value={option} id={`option-${index}`} className="mt-1" />
                  <Label
                    htmlFor={`option-${index}`}
                    className="flex-1 cursor-pointer text-lg text-gray-200 leading-relaxed"
                  >
                    {option}
                  </Label>
                </div>
        ...
        <div className="flex justify-between items-center">
          <Button
            ...
            className="border-gray-600 text-gray-300 hover:bg-white/10 px-8 py-3 bg-transparent"
          >
            Previous
          </Button>
          ...
          <Button
            ...
            className="bg-gradient-to-r from-blue-600 to-purple-600 hover:from-blue-700 hover:to-purple-700 neon-glow px-8 py-3 text-lg"
          >
